...
echo "running /usr/local/bin/wp config create --path=. --dbname=${DBNAMEg} --dbuser=${DBUSER} --dbpass=${DBPASS} --dbhost=${DBHOST} --skip-check"
...
/usr/local/bin/wp core install --url="https://${DOMAIN}" --title="${DOMAIN}" --admin_user="${DBUSER}" --admin_password="${DBPASS}" --dbhost="${DBHOST}" --admin_email="admin@${DOMAIN}" --skip-email