...
export USERNAME="$(aws secretsmanager get-secret-value --secret-id ${ENV}-lev-wordpress-rds-user |  jq -r .SecretString )"
export PASSWORD="$(aws secretsmanager get-secret-value --secret-id ${ENV}-lev-wordpress-rds-password |  jq -r .SecretString )"
cd /tmp
